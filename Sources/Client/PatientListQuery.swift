...
open class PatientListQuery {
	...
	func execute(onServer server: FHIRServer, order: PatientListOrder, callback: @escaping (SwiftFHIR.Bundle?, FHIRError?) -> Void) {
		if isDone {
			callback(nil, nil)
			return
		}
		
		let cb: (SwiftFHIR.Bundle?, FHIRError?) -> Void = { bundle, error in
